...
pub async fn parse_user(name: &str, guild_id: Option<&GuildId>, context: Option<&Context>) -> Option<UserId> {
    ...
    let cached_guild = match guild_id.to_guild_cached(&context).await {
        Some(guild) => guild,
        None => return None
    };
