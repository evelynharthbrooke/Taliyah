...
pub fn version(ctx: &mut Context, message: &Message) -> CommandResult {
    ...
    let rust_version = if cfg!(target_os = "windows") {
        Command::new("cmd").arg("/C rustc -Vv").output().expect("failed to get rust version")
    } else {
        Command::new("sh").arg("rustc -Vv").output().expect("failed to get rust version")
    };

    let cargo_version = if cfg!(target_os = "windows") {
        Command::new("cmd").arg("/C cargo -Vv").output().expect("failed to get cargo version")
    } else {
        Command::new("sh").arg("cargo -Vv").output().expect("failed to get cargo version")
    };
