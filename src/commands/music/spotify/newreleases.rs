use aspotify::{AlbumSimplified, CountryCode};
...
pub async fn newreleases(context: &Context, message: &Message, args: Args) -> CommandResult {
    ...
    if market.len() > 2 || market.len() < 2 {
        message.channel_id.say(context, "You did not provide a valid market name. Name cannot be more or less than 2 characters.").await?;
        return Ok(())
    }
    ...
    let nr_items = new_releases.data.items.iter().map(|album: &AlbumSimplified| {
